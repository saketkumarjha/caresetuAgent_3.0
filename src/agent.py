...
# Add parent directory to path when running directly from src directory
current_dir = os.path.dirname(os.path.abspath(__file__))
parent_dir = os.path.dirname(current_dir)
if parent_dir not in sys.path:
    sys.path.insert(0, parent_dir)

# Import modules - use relative imports when running from src directory
try:
    # Try direct imports first (when running from src directory)
    from config import config
    from core.stt_config import create_assemblyai_stt
    from core.conversation_learning import ConversationLearningEngine, LearningType, ConfidenceLevel
except ImportError:
    # Fall back to absolute imports (when running from project root)
    from src.config import config
    from src.core.stt_config import create_assemblyai_stt
    from src.core.conversation_learning import ConversationLearningEngine, LearningType, ConfidenceLevel

# These imports should work from either location
from knowledge.engines.simple_rag_engine import SimpleRAGEngine
...
Remember: You're representing careSetu with full appointment booking capabilities. Be efficient and helpful while maintaining professionalism.""",
    ...
    def _initialize_rag_components(self):
        ...
        try:
            # Initialize unified knowledge base (without ChromaDB)
            ...
            # Initialize RAG engine (simple text-based search without vector embeddings)
